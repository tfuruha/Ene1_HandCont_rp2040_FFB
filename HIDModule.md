...
以下のデータを 2ms 周期 (`HIDREPO_INTERVAL_MS`) で送信する。全軸 16bit の高解像度通信を行う。

### 3.1 軸入力 (Axes)
| 軸名 | 役割 | レポート値 (16bit) | 元データ |
| :--- | :--- | :--- | :--- |
| Steering (Z) | ハンドル角度 | -32767 ～ 32767 | MF4015 エンコーダ値 |
| Accelerator (Rx) | アクセル量 | -32767 ～ 32767 | ADC0 サンプリング値 |
| Brake (Ry) | ブレーキ圧 | -32767 ～ 32767 | ADC2 サンプリング値 |

### 3.2 ボタン入力 (Buttons)
- **構成**: 16ボタン（現状2ボタン使用）
- **Button 1**: Shift Up (GPIO 14)
- **Button 2**: Shift Down (GPIO 15)
